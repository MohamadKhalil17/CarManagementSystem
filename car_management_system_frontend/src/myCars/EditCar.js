...
export default function EditCar() {
    ...
    useEffect(() => {
      ...
      const onSubmit = async (e) => {
        e.preventDefault();
        await axios.put(`http://localhost:8090/car/editCar/${id}`, car);
        navigate(-1);
      };
    
      const loadCar = async () => {
        const myCar = await axios.get(`http://localhost:8090/car/getCar/${id}`);
        setCar(myCar.data);
      };
      ...
      <div className='container'>
        <div className='raw'>
          <div className='col-md-6 offset-md-3 border rounded p-4 mt-2 shadow'>
            <h2 className='text-center m-4 text-primary' >Edit Car</h2>
